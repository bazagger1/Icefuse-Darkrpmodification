...
File Path:   addons/[server]_darkrpmodification/lua/weapons/icefuse_master_keypad_cracker/shared.lua
...
local master_keypad_crack_time = CreateConVar("master_keypad_crack_time", "10", {FCVAR_ARCHIVE}, "Seconds for keypad cracker to crack keypad")
...
function SWEP:Initialize()
	...
	if SERVER then
		...
		self:SetCrackTime( master_keypad_crack_time:GetInt() )
	end
end	
...
function SWEP:Succeed()
	...
	if SERVER and IsValid(ent) and tr.HitPos:Distance(self.Owner:GetShootPos()) <= 50 and ent.IsKeypad then
		ent:Process(true)
		ent:EmitSound(table.Random(shFireConfig.bombPlant),75,100,1,CHAN_AUTO)
